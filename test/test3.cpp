...
enum ParamTarget { PT_NONE, PT_MUSIC_GATE, PT_BASS, PT_TREBLE, PT_PALETTE, PT_FLASHSET};
...
const char* ARROW_MODE_NAMES[] = {
  "FlashColor", "MusicGate", "BassGate", "TrebleGate", "Brightness"
};
...
const int     DEFAULT_MUSIC_GATE  = 50;
const int     DEFAULT_BASS_GATE   = 130;
const int     DEFAULT_TREBLE_GATE = 130;
...
const uint8_t BASS_GATE_N   = (uint8_t)map(BASS_GATE_THRESH,   0, 900, 0, 255);
const uint8_t TREBLE_GATE_N = (uint8_t)map(TREBLE_GATE_THRESH, 0, 900, 0, 255);
...
// ---- Flash color cycling (via Up/Down arrows) ----
...
// ==== MUSIC PALETTES (1..9) ====
uint8_t musicPaletteIndex = 0; // 0-based
...
// -- Flash (touch) red fade overlay
// -- Flash (touch/key) color fade overlay
// -- Flash (touch/key) two-color fade overlay
...
void handlePotentiometer() {
  ...
  if (allowB) {
    if (potMode == PM_BRIGHT_MUSIC) {
      MUSIC_GATE_THRESH = mapGate(rawB);
    } else { // PM_BASS_TREBLE
      TREBLE_GATE_THRESH = mapGate(rawB);
    }
  }
}
...
void fx_confetti() {
    ...
    // // THRESHOLD HOOK (disabled): uncomment to gate this effect
  // if (musicPaletteIndex == DARK_PALETTE_INDEX) {
  //   int peak = 0; for (int b=0;b<7;b++) peak = max(peak,(int)smoothBands[b]);
  //   if (peak < MUSIC_GATE_THRESH) { fill_solid(leds1,NUM_LEDS,CRGB::Black); fill_solid(leds2,NUM_LEDS,CRGB::Black); return; }
  // }
...
void fx_paletteFlow() {
  ...
  // ---------- AUDIO (normalized) ----------
  uint8_t bassN   = bandNorm[1];
  uint8_t trebleN = bandNorm[5];
  ...
  float target = audioPeakN / 255.0f;
  ...
  unsigned long now = millis();
  if (normTo900(audioPeakN) >= LASER_GATE_THRESH) {
...
void handleInputs() {
  while (Serial.available() > 0) {
    ...
    if (escState == ESC_IDLE) {
      ...
      if (c == 'A' || c == 'B' || c == 'C' || c == 'D') {
        // Right/Left switch target among the 3 gates
        if (c == 'C' || c == 'D') {
          const uint8_t FIRST = ARROW_MUSIC_GATE;
          const uint8_t LAST  = ARROW_TREBLE_GATE;
          if (c == 'C') { // Right
...
static void drawSettingsMusic() { const char* items[3] = {"Music Gate", "Bass Gate", "Treble Gate"}; int vals[3] = {MUSIC_GATE_THRESH, BASS_GATE_THRESH, TREBLE_GATE_THRESH}; display.clearDisplay(); display.setTextSize(1); display.setTextColor(SSD1306_WHITE); display.setCursor(0,0); display.println("Music Settings"); for (uint8_t i=0;i<3;i++){ if (i==musicCursor) display.print("> "); else display.print(" "); display.print(items[i]); display.print(": "); display.println(vals[i]); } display.setCursor(0,56); display.print("E/G:Select F:Adjust H:Back"); display.display(); }
